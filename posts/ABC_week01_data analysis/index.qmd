...
이 프로젝트를 진행을 위해 1주차로 기초 시계열 분석을 진행해봤습니다.
...
**참고:** 시계열 분해를 실행하려면 `statsmodels` 라이브러리가 설치되어 있어야 합니다. (`pip install statsmodels`) 위 코드 셀의 `eval` 옵션이 `false`로 설정되어 있어, 실행하려면 `true`로 변경하고 라이브러리를 설치해야 합니다.
