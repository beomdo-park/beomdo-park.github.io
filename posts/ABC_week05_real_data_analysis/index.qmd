...
# matplotlib 한글 폰트 설정 (Week04 내용 재활용)
import os
...
데이터는 'StartDate', 'Value', 'units' 등의 컬럼으로 구성되어 있습니다. 시계열 분석을 위해 'StartDate'를 datetime 형식으로 변환하고 인덱스로 설정한 뒤, 'Value' 컬럼만 사용하겠습니다. 데이터의 양이 매우 크므로, 실험을 위해 2018년 1월 한 달 치 데이터만 샘플링하여 사용하겠습니다.
...
# 'Value' 컬럼만 선택
df_value = df[['Value']].copy()

# 2018년 1월 데이터만 선택
df_sample = df_value['2018-01-01':'2018-01-31']

# 데이터 시각화
plt.figure(figsize=(15, 6))
plt.plot(df_sample.index, df_sample['Value'], label='전력 사용량 (2018년 1월)')
plt.title('시간에 따른 전력 사용량')
plt.xlabel('날짜')
plt.ylabel('사용량 (Wh)')
plt.legend()
plt.grid(True)
...
scaled_data = scaler.fit_transform(df_sample['Value'].values.reshape(-1, 1)).flatten()
...
print("모델 학습 시작...")
epochs = 20 # 실제 데이터는 양이 많으므로 epoch 수를 줄여서 진행
