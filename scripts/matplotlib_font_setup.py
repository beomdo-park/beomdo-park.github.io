...
def setup_korean_fonts():
    ...
    try:
        ...
        if os_name == 'Windows':
            font_name_to_set = 'NanumGothic' # 사용자가 Windows에 나눔고딕을 설치했다고 가정
        ...
        if font_name_to_set:
            ...
            if font_name_to_set in available_fonts:
            ...
            elif os_name == 'Linux' and 'NanumSquare' in available_fonts: # Alternative Nanum font
                ...
                if nanum_fonts:
                    ...
                    warnings.warn(f"'{font_name_to_set}' not found on {os_name}. Korean text may not render correctly. Ensure 'fonts-nanum' is installed and Matplotlib's cache is updated.")
            else:
                 warnings.warn(f"'{font_name_to_set}' not found on {os_name}. Korean text may not render correctly.")


        plt.rcParams['axes.unicode_minus'] = False # 마이너스 부호 깨짐 방지

        if font_family_set:
            print(f"Matplotlib font successfully set to '{font_name_to_set}' for Korean on {os_name}.")
        elif os_name != 'Linux': # Linux는 위에서 이미 경고했을 수 있음
             warnings.warn(f"Could not set a specific Korean font on {os_name}. Defaulting to system fonts. Korean text might not display correctly.")


    except Exception as e:
        warnings.warn(f"Error setting up Matplotlib font: {e}\\n"
